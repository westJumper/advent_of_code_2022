...
var groundY = overflowYPosition + 2

function createGround(minX, maxX, groundY){
  var ground = []
   for(i=minX; i<= maxX;i++){
    ground.push([i, groundY])
   }
   return ground
}

//displayCave(rocks, sands, [0,0]) // display without sand
releaseSandFlow(sandStart)
...
function releaseSandFlow(start){
  ...
  while(sandInGrid) { 
    ...
    while(sandCanMove){
      ...
      var positionToMove = sandCanMoveToPosition(currentSandPosition, occupiedPositions,groundY)
      ...
      if(positionToMove.length == 0){ 
        ...
        sandPath.push(currentSandPosition)
        currentSandPosition = positionToMove
      }

      // ground?
      if(sandCanMoveToPosition(start, occupiedPositions, groundY) == 0){
        sandCanMove = false;
        sandInGrid = false
      }
...
// make sand positions unique
// this could be optimised so that it does not use array in the first place - not necesarry :)
var set  = new Set(sands.map(JSON.stringify));
var arr2 = Array.from(set).map(JSON.parse);

console.log("sand count when sand cannot pour from sand anymore (result of solution 2): " + (arr2.length+1))

}

function sandCanMoveToPosition(currentSandPosition, occupiedPositions, groundY){
      ...
      var bottomIsGround = currentSandPosition[1]+1 == groundY

      if(bottomIsGround) return []
